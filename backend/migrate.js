...
async function migrate() {
    ...
    const resultStmt = db.prepare('INSERT INTO results (pilot, id_circuito, date, division, pos_clasificacion, pos_final, tiempo_vuelta, es_vuelta_rapida, condicion, investigating, replaces, tiempo_qualy, temporada) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        for (const line of resultsLines) {
            ...
            if (r.length >= 8) {
                resultStmt.run(
                    ...
                    r[6], r[7]?.toUpperCase() === 'TRUE' ? 1 : 0, r[8] || 'Seco', parseInt(r[9]) || 0, r[10] || null, r[11] || null, r[12] || '2026'
                );
            }
        }
    })();
